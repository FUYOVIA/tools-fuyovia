...
export async function POST(request: NextRequest) {
  try {
    const auth = await authenticateAndCheckCredits(request, 'social-media-generator')
    if (auth.error) {
...
Key: Twitter rewards replies and retweets. Write to spark conversation, not just broadcast.`,
    ...
    await deductCredits(auth.userId, 'social-media-generator')
...
export async function GET() {
  return NextResponse.json({ status: 'Social Media Generator API is running' })
}
