...
export async function POST(request: NextRequest) {
  try {
    const auth = await authenticateAndCheckCredits(request, 'seo-blog-generator')
    if (auth.error) {
...
Make this post genuinely valuable - not fluff, but actual insights a reader would bookmark and share. Include data points, examples, or frameworks where relevant.`
    ...
    await deductCredits(auth.userId, 'seo-blog-generator')
...
export async function GET() {
  return NextResponse.json({ status: 'SEO Blog Generator API is running' })
}
