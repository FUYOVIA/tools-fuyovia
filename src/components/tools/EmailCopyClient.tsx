...
export default function EmailCopyClient({ previewMode = false }: EmailCopyClientProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await authFetch('/api/email-copy-generator', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productName: product, goal, emailType, audience }),
      })
      if (res.status === 401) {
      ...
      if (data.error) {
        ...
        // API returns { success: true, data: "markdown text", emailType }
        // Display as plain text since API generates full markdown output
        setOutput(data)
      }
    } catch {
      ...
      {resultText && (
        <div className="bg-neutral-50 border border-neutral-200 rounded-3xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-neutral-800">Generated Email Copy</h3>
            <button
